...
#if SIZEOF_VOID_P == 8
    /* Warning generated by Cython that will show when importing e.g. numpy. */
    PyList_SET_ITEM(filters, pos++,
                    create_filter_with_message(PyExc_RuntimeWarning, &PyId_ignore, "_frozen_importlib",
                                               /* string matches exactly (see below `check_matched`) */
                                               "builtins.type size changed, may indicate binary incompatibility. Expected 880 from C header, got 896 from PyObject"));
#else
